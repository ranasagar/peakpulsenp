...
const fallbackHeroSlide: HeroSlide = {
  ...
  description: "Experience the fusion of ancient Nepali artistry and modern streetwear. Content is loading, please wait.",
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        errorBody = errorData.error || errorData.message || await res.text();
      } catch (e) {
         errorBody = await res.text(); // Fallback if response is not JSON
      }
    ...
    const processedHeroSlides = (Array.isArray(jsonData.heroSlides) && jsonData.heroSlides.length > 0)
      ? jsonData.heroSlides.map((slide: Partial<HeroSlide>, index: number) => ({
          ...
          imageUrl: slide.imageUrl, // Keep as is, undefined if not provided
          videoId: slide.videoId,   // Keep as is, undefined if not provided
...
export default function HomePage() {
  ...
  const activeHeroSlides = content.heroSlides || [fallbackHeroSlide];
  ...
  const currentHeroSlideData = activeHeroSlides[currentSlide] || fallbackHeroSlide;
  const heroVideoId = currentHeroSlideData.videoId || content.heroVideoId;
  const heroImageUrl = currentHeroSlideData.imageUrl || content.heroImageUrl;
        ...
        {activeHeroSlides.map((slide, index) => (
          ...
          >
            <div className="absolute inset-0 z-0 pointer-events-none"> 
              {(slide.videoId || content.heroVideoId) ? (
                <>
                  <iframe
                    className="absolute top-1/2 left-1/2 w-full h-full min-w-[177.77vh] min-h-[56.25vw] transform -translate-x-1/2 -translate-y-1/2"
                    src={`https://www.youtube.com/embed/${slide.videoId || content.heroVideoId}?autoplay=1&mute=1&loop=1&playlist=${slide.videoId || content.heroVideoId}&controls=0&showinfo=0&autohide=1&modestbranding=1&playsinline=1&enablejsapi=1`}
              ...
              ) : (slide.imageUrl || content.heroImageUrl) ? (
                <>
                  <Image
                    src={slide.imageUrl || content.heroImageUrl!}
