...
export default function ProductDetailPage({ params: paramsPromise }: { params: Promise<{ slug: string }> | { slug: string } }) {
  ...
  const fetchProductData = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // console.error("[ProductDetail] Error from API:", errorMsg); // Keep for debugging if needed
        setError(errorMsg);
        setProduct(null);
        setIsLoadingProduct(false); 
        return; 
  ...
  const fetchReviewsForProduct = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Could not load reviews for this product.", variant: "destructive"});
    } finally {
  ...
  // CompareAtPrice should only be shown if it's higher than the displayPrice
  const displayCompareAtPrice = product?.compareAtPrice && product.compareAtPrice > displayPrice 
                                ? product.compareAtPrice 
                                : (selectedVariant?.costPrice !== undefined && selectedVariant?.costPrice !== null && product?.price > displayPrice ? product.price : undefined);
  ...
  const isOutOfStock = useMemo(() => {
    ...
    return (product?.stock !== undefined && product.stock !== null && product.stock <= 0 && (!product?.variants || product.variants.length === 0));
  ...
  const handleToggleWishlist = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || errorData.rawSupabaseError?.message || `Failed to update wishlist`);
      }
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.message || errorData.rawSupabaseError?.message || "Failed to submit review.");
        }
        toast({ title: "Review Submitted!", description: "Thank you for your feedback. Your review is pending approval." });
        setReviewRating(0);
        setReviewTitle('');
        setReviewComment('');
        // Optionally re-fetch reviews if you want to show pending ones or optimistic updates
        // fetchReviewsForProduct(); 
    } catch (error) {
        toast({ title: "Review Submission Failed", description: (error as Error).message, variant: "destructive" });
    } finally {
  ...
  if (isLoadingProduct || authIsLoading) {
    return (
      <MainLayout>
  ...
  const currentReviewCount = reviews.length > 0 ? reviews.length : (product.reviewCount || 0);
    ...
    <MainLayout>
      <div className="container-wide section-padding">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div>
            ...
            <div className="flex items-center mb-1">
              <RatingStars rating={currentAverageRating} size={20} />
              <span className="ml-2 text-sm text-muted-foreground">({currentReviewCount} reviews)</span>
            </div>
                            ...
                            >
                                {product.availablePrintDesigns.map(design => (
                                    <Label 
                                        ...
                                        className={cn(
                                          "border rounded-md p-2 cursor-pointer hover:border-primary transition-all flex flex-col items-center space-y-2",
        ...
        <section id="reviews-section" className="space-y-12">
          ...
          {isLoadingReviews ? (
             <div className="flex justify-center items-center py-10"><Loader2 className="h-8 w-8 animate-spin text-primary"/> <p className="ml-3 text-muted-foreground">Loading reviews...</p></div>
          ) : reviews && reviews.length > 0 ? (
            <Card className="shadow-lg">
              <CardHeader>
                <div className="flex items-center gap-4">
                  <RatingStars rating={currentAverageRating} size={28}/>
                  <div >
                    <CardTitle className="text-2xl">{currentAverageRating.toFixed(1) || 'N/A'} out of 5</CardTitle>
                    <p className="text-sm text-muted-foreground">Based on {currentReviewCount} reviews</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                  ...
                  <div key={review.id} className="pb-6 border-b last:border-b-0 last:pb-0">
                    <div className="flex items-start space-x-3 mb-2">
                      <Avatar className="h-10 w-10 mt-0.5">
                        ...
                        <AvatarFallback>{review.user_name ? review.user_name.charAt(0).toUpperCase() : 'U'}</AvatarFallback>
                      </Avatar>
                      <div>
                      ...
                      <p className="text-xs text-muted-foreground ml-auto">{format(new Date(review.createdAt), 'MMM d, yyyy')}</p>
                    </div>
                ...
                {reviews.length > 3 && (
                  <div className="text-center pt-4">
                      <Button variant="outline">View all {currentReviewCount} reviews (UI Only)</Button>
                  </div>
                )}
              </CardContent>
            </Card>
