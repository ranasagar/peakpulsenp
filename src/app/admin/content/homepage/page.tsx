...
const homepageContentSchema = z.object({
  heroSlides: z.array(heroSlideSchema).min(0, "At least one hero slide is recommended.").optional(), // Allow empty array
  artisanalRootsTitle: z.string().min(3, "Artisanal roots title must be at least 3 characters.").optional().or(z.literal('')),
  artisanalRootsDescription: z.string().min(5, "Artisanal roots description must be at least 5 characters.").optional().or(z.literal('')),
      ...
      try {
        ...
        form.reset({
          heroSlides: data.heroSlides && data.heroSlides.length > 0 
            ? data.heroSlides.map(slide => ({ ...defaultHeroSlide, ...slide, id: slide.id || `hs-${Date.now()}` })) 
          ...
          socialCommerceItems: data.socialCommerceItems 
            ? data.socialCommerceItems.map(item => ({ ...defaultSocialCommerceItem, ...item, id: item.id || `sc-${Date.now()}` })) 
            : [],
  ...
  if (isLoading) {
    return (
      <Card className="shadow-lg">
        <CardHeader><CardTitle className="text-2xl">Edit Homepage Content</CardTitle><CardDescription>Loading content...</CardDescription></CardHeader>
      ...
      <ScrollArea className="max-h-[75vh] pr-6">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <fieldset className="space-y-4 p-4 border rounded-md">
               <legend className="text-xl font-semibold px-1 mb-2">Standalone Hero Background (If not using Carousel, or as ultimate fallback)</legend>
                 <FormField control={form.control} name="heroImageUrl" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Used if no carousel slides are active or if you prefer a single static image. Tip: For quick uploads, try free sites like ImgBB.com or Postimages.org.</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )} />
