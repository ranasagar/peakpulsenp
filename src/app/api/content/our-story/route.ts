...
const fallbackContent: OurStoryContentData = {
  hero: { title: "Our Story (Default Fallback)", description: "Weaving together heritage and vision to share with the world." },
  mission: { title: "Our Mission (Default Fallback)", paragraph1: "Elevating craftsmanship and connecting cultures through unique apparel.", paragraph2: "Every piece tells a story of tradition and modernity." },
  craftsmanship: { title: "The Art of Creation (Default Fallback)", paragraph1: "Honoring ancient techniques with a commitment to quality.", paragraph2: "Sustainably sourced materials form the heart of our designs." },
  valuesSection: { title: "Our Values: Beyond the Seams (Default Fallback)" },
  joinJourneySection: { title: "Join Our Journey (Default Fallback)", description: "Follow us and be part of the Peak Pulse story." },
...
export async function GET() {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ ...fallbackContent, error: `Failed to load Our Story content. Supabase: ${error.message}` }, { status: 500 });
    }
